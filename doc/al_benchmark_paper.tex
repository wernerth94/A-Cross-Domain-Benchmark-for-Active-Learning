...
\begin{minipage}{0.4\linewidth}
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			\Ensure Retrain, margin
			\Require $\mathcal{U}^{(i)}, \mathcal{L}^{(i)}, \tau, \hat y_\theta$ 
			\State $\text{acc}^{(i)} \gets \operatorname{Retrain}(\mathcal{L}^{(i)})$ 
			\State $u^{(i)} \underset{\tau}{\sim} \text{unif}(1:|\mathcal{U}^{(i)}|)$
			\State $r^* \gets 0$
			\For{$t := 1 \ldots \tau$}
			\State $\mathcal{K} \gets \mathcal{L}^{(i)} \cup \{(u^{(i)}_t, y_t)\}$
			\State $\text{acc} \gets \operatorname{Retrain}(\mathcal{K}, \hat y_\theta)$  
			\State $r \gets \text{acc} - \text{acc}^{(i)}$
			\If{$r > r^*$} 
			\State $r^* \gets r$
			\State $t^* \gets t$
			\EndIf  
			\If{$r^* = 0$}
			\State $t^* \gets \operatorname{margin}(u^{(i)}, \hat y_\theta)$
			\EndIf
			\EndFor
			\Return $t^*$
		\end{algorithmic}
...
\begin{minipage}{0.59\linewidth}
	In Alg. \ref{alg:oracle} $\operatorname{Retrain}(\mathcal{L}^{(i)}, \hat y_\theta)$ trains the classification model $\hat y_\theta$ and returns the accuracy on the test set $\mathcal{D}_\text{test}$. \\ 
