...
class FlowALGame(ALGame):
    def reset(self, *args, **kwargs) -> list:
        with torch.no_grad():
            self.n_interactions = 0
            self.added_images = 0
            self.classifier = zuko.flows.NSF(self.dataset.x_labeled.size(-1), self.dataset.y_labeled.size(-1),
                                             transforms=3, hidden_features=[128] * 3)
            self.classifier = self.classifier.to(self.device)
            self.initial_weights = self.classifier.state_dict()
            self.optimizer = torch.optim.AdamW(self.classifier.parameters(), lr=1e-3)
            self.reset_al_pool()
            self.class_vectors = []
            for c in range(self.dataset.n_classes):
                cls_list = [0.0]*self.dataset.n_classes
                cls_list[c] = 1.0
                one_hot = torch.Tensor([cls_list]*64).reshape(64, self.dataset.n_classes)
                self.class_vectors.append(one_hot)
        # first training of the model should be done from scratch
        self._fit_classifier(from_scratch=True)
        self.initial_test_accuracy = self.current_test_accuracy
        return self.create_state()
    ...
    def _fit_classifier(self, epochs=50, from_scratch=False):
        ...
        for e in range(epochs):
            ...
            for i, (batch_x, batch_y) in enumerate(train_dataloader):
                self.optimizer.zero_grad()
                loss_value = - self.classifier(batch_y).log_prob(batch_x)
            ...
            with torch.no_grad():
                ...
                loss_sum = 0.0
                for batch_x, batch_y in val_dataloader:
                    class_loss = - self.classifier(batch_y).log_prob(batch_x)
        ...
        with torch.no_grad():
            ...
            correct = 0.0
            for batch_x, batch_y in test_dataloader:
                y_vectors = torch.cat([v[:len(batch_x)] for v in self.class_vectors])
                x_vectors = batch_x.repeat(len(self.class_vectors), 1)
                yHat = self.classifier(y_vectors).log_prob(x_vectors)
                yHat = torch.stack([yHat[:len(batch_x)], yHat[len(batch_x):]], dim=0).T
                predicted = torch.argmax(yHat, dim=1)
                correct += (predicted == torch.argmax(batch_y, dim=1)).sum().item()
            accuracy = correct / len(self.dataset.x_test)
            reward = accuracy - self.current_test_accuracy
